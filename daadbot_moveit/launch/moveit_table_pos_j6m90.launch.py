"""MoveIt 2 move_group + RViz for the urdf_table_pos_j6m90 model.

This is the planning layer only. It expects a sim (or hardware) already running
with the JointTrajectoryControllers from pos_trajectory_controller.yaml, which
moveit_controllers_pos.yaml maps to FollowJointTrajectory.

For the full stack in one command use:
    ros2 launch daadbot_bringup sim_robot_table_pos_j6m90_moveit.launch.py
Standalone (sim started elsewhere):
    ros2 launch daadbot_moveit moveit_table_pos_j6m90.launch.py
"""
import os
...
def generate_launch_description():
    ...
    move_group_node = Node(
        ...
        parameters=[
            ...
            os.path.join(pkg_moveit, "config", "moveit_controllers_pos.yaml"),
        ],
