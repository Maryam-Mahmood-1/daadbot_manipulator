...
def main():
    ...
    ax_v, ax_mu = fig.add_subplot(gs[1, 0]), fig.add_subplot(gs[1, 1])
    ...
    for i in range(2):
        ...
        lr, = ax.plot([], [], 'k--', alpha=0.5, label='Ref')
        lt, = ax.plot([], [], color=colors[i], lw=2, label='Actual')
        lns_ref.append(lr); lns_traj.append(lt)
        ax.set_title(node.labels[i]); ax.set_xlim(-1.85, 1.85); ax.set_ylim(-1.95, 1.95); ax.grid(True)
    ...
    def update(frame):
        with node.lock:
            for i, lbl in enumerate(node.labels):
                ...
                lns_v[i].set_data(d['t'], d['V']); lns_mu[i].set_data(d['t'], d['mu'])
            
            ref_lbl = node.labels[1]
            if node.log[ref_lbl]['t']:
                t = node.log[ref_lbl]['t']
                ax_v.set_xlim(t[0], t[-1]); ax_mu.set_xlim(t[0], t[-1])
                ax_v.set_ylim(0, max(max(node.log[node.labels[0]]['V'][-10:] if node.log[node.labels[0]]['V'] else [1]), 1.0) * 1.5)
                ax_mu.set_ylim(0, 15)
        return lns_traj + lns_ref + lns_v + lns_mu
