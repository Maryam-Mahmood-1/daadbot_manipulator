...
class SINDyComparisonNode(Node):
    def __init__(self):
        ...
        # Keep internal labels same but we will skip plotting index 0
...
def main():
    ...
    for i in range(3):
        # Top Row: Individual Plot (Still showing all 3 subplots for individual trajectory viewing)
        ...
        # Bottom Row & Combined Overlay: Only if NOT Feedback Linearization (i != 0)
        visible = (i != 0)
        lc, = ax_combined.plot([], [], color=colors[i], lw=(4.5 - i), label=node.labels[i], alpha=0.8, visible=visible)
        lns_combined.append(lc)
        
        lv, = ax_v.plot([], [], color=colors[i], label=node.labels[i], lw=1.5, visible=visible)
        lm, = ax_mu.plot([], [], color=colors[i], label=node.labels[i], lw=1.5, visible=visible)
        lns_v.append(lv); lns_mu.append(lm)

    ax_combined.set_title("Comparison Overlay (CLF Methods Only)")
    ...
    def update(frame):
        with node.lock:
            for i, lbl in enumerate(node.labels):
                ...
                # Individual Trajectory Axes (Update all)
                lns_traj[i].set_data(d['x'], d['y'])
                lns_ref[i].set_data(d['xd'], d['yd'])
                
                # Comparison and Metrics (Update only for CLF/Robust)
                if i != 0:
            ...
            # Adjust axes based on visible data only (i=1 and i=2)
            active_labels = node.labels[1:]
            all_times = [node.log[l]['t'] for l in active_labels if node.log[l]['t']]
            if all_times:
            ...
            v_data = [v for l in active_labels for v in node.log[l]['V'][-30:] if node.log[l]['V']]
            if v_data: ax_v.set_ylim(0, max(v_data) * 1.2)
            
            mu_data = [m for l in active_labels for m in node.log[l]['mu'][-30:] if node.log[l]['mu']]
