...
# ==========================================
# 1. ENHANCED ROBOTIC LIBRARY (No raw angles, added Taylor terms)
# ==========================================
def build_library(X, U):
    ...
    # [UPDATED] Removed q0 and q1 to support continuous rotation safely
    H_x = np.hstack([
        np.ones((X.shape[0], 1)), 
        dq0, dq1, 
        s1, c1, s2, c2, s12, c12,
    ...
    # [UPDATED] Added c2**3 and c2**4 to better fit the 1/det(M) rational functions
    Theta_g0 = np.hstack([u0, u0 * c2, u0 * s2, u0 * c12, u0 * c2_sq, u0 * s2_sq, u0 * (c2**3), u0 * (c2**4)])
    Theta_g1 = np.hstack([u1, u1 * c2, u1 * s2, u1 * c12, u1 * c2_sq, u1 * s2_sq, u1 * (c2**3), u1 * (c2**4)])
    
    return np.hstack([H_x, Theta_g0, Theta_g1])

# ==========================================
# 2. SINDY TRAINING (Dense Ridge Regression Bypass)
# ==========================================
def sparse_regression(Theta, Y, threshold=0.0, alpha=1e-6):
    ...
    # Bypass the STLSQ thresholding loop completely if threshold is 0.0
    if threshold > 0.0:
        for _ in range(15):
...
def train_and_calibrate(train_df, cal_df, delta=0.1, threshold=0.0, alpha=1e-6):
    ...
    print("Training State-Space SINDy Model (Dense Fit)...")
    Theta_train = build_library(X_train, U_train)
    Xi = sparse_regression(Theta_train, Y_train, threshold=threshold, alpha=alpha)
...
if __name__ == "__main__":
    csv_path = "/home/maryammahmood/xdaadbot_ws/2link_pe_dataset2.csv"
    train_df, cal_df, _ = load_and_split_data(csv_path)
    
    # Run pure Dense Fit (threshold=0.0)
    Xi, q_val = train_and_calibrate(train_df, cal_df, threshold=0.0)
    
    print(f"\nModel Shape (Features x Targets): {Xi.shape}")
    print(f"Safety Bound (q_1-delta): {q_val:.6f}")
    
    np.save("/home/maryammahmood/xdaadbot_ws/sindy_Xi_state_space2.npy", Xi)
    with open("/home/maryammahmood/xdaadbot_ws/q_quantile_state_space2.txt", "w") as f:
        f.write(str(q_val))
