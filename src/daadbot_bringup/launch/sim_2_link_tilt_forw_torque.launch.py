...
def generate_launch_description():
    ...
    xacro_file = os.path.join(pkg_desc, 'urdf/2_link_urdf/2link_robot_fixed.urdf.xacro')


    # 2. Gazebo Simulation
    gazebo = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_desc, "launch", "gazebo_2link_tilt_effort.launch.py")
        )
    )
    ...
    controller = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_controller, "launch", "effort_forw_controller_2link_tilt.launch.py")
        ),
