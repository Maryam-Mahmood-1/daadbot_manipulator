...
from model_training_2 import build_library
import os
...
if __name__ == "__main__":
    ...
    try:
        # Load the CSV purely with Pandas
        df = pd.read_csv(csv_path)
        test_df = df[df['split'] == 'test'].copy()
        
        if len(test_df) == 0:
            print("Error: No 'test' split found. Did you run the new data generation script?")
            exit()
            
        Xi = np.load("/home/maryammahmood/xdaadbot_ws/sindy_Xi_state_space2.npy")
        with open("/home/maryammahmood/xdaadbot_ws/q_quantile_state_space2.txt", "r") as f:
            q_val = float(f.read())
            
        generate_detailed_plots(test_df, Xi, q_val)
        print("Evaluation complete. Diagnostic plots saved.")
