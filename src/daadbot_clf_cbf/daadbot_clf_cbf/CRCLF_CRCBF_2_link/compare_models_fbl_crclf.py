...
# --- MODULAR IMPORTS ---
# Ensure dynamics_wrapper.py is accessible in your workspace
from daadbot_clf_cbf.CRCLF_CRCBF_2_link.dynamics_wrapper import LearnedRobotDynamics
from daadbot_clf_cbf.CRCLF_CRCBF_2_link.trajectory_generator import TrajectoryGenerator
from daadbot_clf_cbf.CRCLF_CRCBF_2_link.resclf_controller import RESCLF_Controller
from daadbot_clf_cbf.CRCLF_CRCBF_2_link.cbf_formulation import CBF_SuperEllipsoid 
...
MODEL_PKL = "robot_dynamics_model.pkl" 
...
class ParallelComparisonNode(Node):
    def __init__(self):
        ...
        # 1. LOAD CALIBRATED QUANTILE FROM LEARNED MODEL
        try:
            with open(MODEL_PKL, "rb") as f:
                saved_data = pickle.load(f)
            self.calibrated_q = saved_data.get("q_quantile", 1.0)
            self.get_logger().info(f"Loaded Calibrated Quantile: {self.calibrated_q:.4f}")
        except Exception as e:
            self.get_logger().error(f"Failed to load pkl: {e}")
            self.calibrated_q = 150.0 # Fallback high value
        ...
        # Modes: 0=F.Lin (No CLF), 1=Nominal Learned (q=0), 2=Robust Learned (q=calibrated)
        self.quantiles = [0.0, 0.0, self.calibrated_q]
        self.labels = ['F. Lin (Baseline)', 'Nominal Learned (q=0)', f'Robust Learned (q={self.calibrated_q:.1f})']
        ...
        for i in range(len(self.labels)):
            ...
            # Controller Model (Learned Regression Model)
            # Replaced RobotDynamics(URDF_NOISY) with LearnedRobotDynamics(MODEL_PKL)
            self.robot_ctrls.append(LearnedRobotDynamics(MODEL_PKL, URDF_TRUE, EE_NAMES))
        ...
        while self.running:
            with self.lock:
                for i in range(len(self.labels)):
                    ...
                    # True Physics Simulation via pin.aba
    ...
    def control_loop(self):
        ...
        for i in range(len(self.labels)):
            ...
            # A. LEARNED DYNAMICS (Replacing noisy URDF with regression model)
            ...
            # B. CONFORMAL ROBUSTNESS CONSTRAINTS
...
def main():
    ...
    # --- PLOTTING SETUP (Visualizing the discrepancy handle) ---
    ...
    ax_v = fig.add_subplot(gs[1, 0]); ax_h = fig.add_subplot(gs[1, 1]); ax_mu = fig.add_subplot(gs[1, 2])
    ax_tau1 = fig.add_subplot(gs[2, 0:2]); ax_tau2 = fig.add_subplot(gs[2, 2])
    ...
    xb = rx * np.sign(np.cos(theta)) * (np.abs(np.cos(theta)) ** (2/n)); yb = ry * np.sign(np.sin(theta)) * (np.abs(np.sin(theta)) ** (2/n))
    ...
    ax_tau1.set_title("Joint 1 Torque"); ax_tau2.set_title("Joint 2 Torque")
